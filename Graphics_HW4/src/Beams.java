...
class BeamsObj {
	   ...
	   BeamsObj(int n, double a, double alpha, String fileName)
	      ...
	      double b = a - 1;
	      P[1] = new Point3D(0, -a, 0);
	      P[2] = new Point3D(0, a, 0);
	      P[3] = new Point3D(-b, a, 0);
	      P[4] = new Point3D(-b, -a, 0);
	      P[5] = new Point3D(0, -a, 1);
	      P[6] = new Point3D(0, a, 1);
	      ...
	      for (int k = 0; k < n; k++) { // Beam k:
	         ...
	         // Shift next rectangle over
	         double xShift = k * a/2;
	         
	         for (int i = 1; i <= 8; i++) {
	            double x = P[i].x + xShift, y = P[i].y;
	            float x1 = (float) (x * cosPhi - y * sinPhi); 
	                  float y1 = (float) (x * sinPhi + y * cosPhi);
